...
def model_check(knowledge, query):
    ...
    def check_all(knowledge, query, symbols, model):
        ...
        # If model has an assignment
        # for each symbol
        if not symbols:

            # If knowledge base is true
            # in model, then query must also be true
            ...
            # Choose one remaining symbol
            remaining = symbols.copy()
            p = remaining.pop()

            # Model where symbol is True
            model_true = model.copy()
            model_true[p] = True

            # Model where symbol is False
